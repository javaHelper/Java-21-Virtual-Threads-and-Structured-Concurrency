...
import org.example.LongRunningTask;
import org.example.LongRunningTask.TaskResponse;
 ...
 * The scope.join() method will wait till the first two "successful" thread
 * forked from the scope terminates and send cancellation requests to the rest
 * of the threads
 */
public class STaskScopeCustomExecutor {
	...
	public static void main(String[] args) throws Exception {
		...
		TaskResponse result = execute(List.of(w1Task, w2Task, w3Task, w4Task, w5Task));
...
class AverageWeatherTaskScope extends StructuredTaskScope<TaskResponse> {
	...
	private final List<Subtask<? extends TaskResponse>> responses = Collections.synchronizedList(new ArrayList<>());
